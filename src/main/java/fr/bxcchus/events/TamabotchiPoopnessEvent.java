...
public class TamabotchiPoopnessEvent extends TimerTask {

    @Override
    public void run() {
        try {
            for (Tamabotchi tamabotchi : TamabotchiManager.getInstance().getTamabotchis()) {
                TamabotchiManager.getInstance().updateTamabotchiPoop(tamabotchi.getId(), tamabotchi.getName(), tamabotchi.getLvl(), tamabotchi.getHp(), tamabotchi.getHappiness(), tamabotchi.getHydration(), tamabotchi.getHunger(), tamabotchi.getRace().getId(), tamabotchi.getRace().getName(), tamabotchi.getPlayer().getUid(), tamabotchi.getPlayer().getUsername(), tamabotchi.getDeath().getId(), tamabotchi.getDeath().getCause(), tamabotchi.getPoopness());
                if (tamabotchi.getPoopness() <= -100) {
                    System.out.println(tamabotchi.getName() + " is dead because he wasn't poop. The owner was: " + tamabotchi.getPlayer().getUsername());
